...
public class GLCube {
  ...
  public void draw(GL10 gl) {
    ...
    if (this.cubeProperties.getShown() && !this.cubeProperties.getDestroyed()) {
      ...
      if (cubeProperties.getMarked()) {
        gl.glColor4f(0.0f, 0.0f, 25.0f, 1.0f);
      } else if (cubeProperties.getErrored()) {
        gl.glColor4f(50.0f, 0.0f, 0.0f, 1.0f);
      }
      ...
      if (GameManager.getInstance().isCleared()) {
        gl.glBindTexture(GL10.GL_TEXTURE_2D, TextureManager.getInstance().getTexture()[TextureManager.TEX_EMPTY]);
      }
      //Bind the texture and draw the vertices as triangle strips
      if (!GameManager.getInstance().isCleared()) {
        gl.glBindTexture(GL10.GL_TEXTURE_2D, TextureManager.getInstance().getTexture()[cubeProperties.getXHint()]);
      }
      gl.glDrawElements(GL10.GL_TRIANGLES, indices_x.length, GL10.GL_UNSIGNED_BYTE, indexBuffer_x);
      if (!GameManager.getInstance().isCleared()) {
        gl.glBindTexture(GL10.GL_TEXTURE_2D, TextureManager.getInstance().getTexture()[cubeProperties.getYHint()]);
      }
      gl.glDrawElements(GL10.GL_TRIANGLES, indices_y.length, GL10.GL_UNSIGNED_BYTE, indexBuffer_y);
      if (!GameManager.getInstance().isCleared()) {
        gl.glBindTexture(GL10.GL_TEXTURE_2D, TextureManager.getInstance().getTexture()[cubeProperties.getZHint()]);
      }
  ...
  public int tryToMark() {
    if (!cubeProperties.getErrored()) {
      if (cubeProperties.getMarked()) {
        cubeProperties.setMarked(false);
        return UNMARK_CUBE;
      } else {
        cubeProperties.setMarked(true);
        return MARK_CUBE;
      }
    }
    return DO_NOTHING;
  ...
  public int tryToDestroy() {
    if (!cubeProperties.getMarked() && !cubeProperties.getErrored()) {
      if (cubeProperties.getSolid()) {
        cubeProperties.setErrored(true);
        return ERROR_CUBE;
      } else {
        cubeProperties.setDestroyed(true);
        return DESTROY_CUBE;
      }
    }
    return DO_NOTHING;
  ...
  public boolean getSolid() {
    return cubeProperties.getSolid();
  }

  public boolean getShown() {
    return cubeProperties.getShown();
  }

  public void setShown(boolean shown) {
    cubeProperties.setShown(shown);
  }

  public boolean getDestroyed() {
    return cubeProperties.getDestroyed();
  }

  public void setDestroyed(boolean destroyed) {
    cubeProperties.setDestroyed(destroyed);
  }

  public boolean getMarked() {
    return cubeProperties.getMarked();
  }

  public void setMarked(boolean marked) {
    cubeProperties.setMarked(marked);
  }

  public boolean getErrored() {
    return cubeProperties.getErrored();
  }

  public void setErrored(boolean errored) {
    cubeProperties.setErrored(errored);
  }

  public int getID() {
    return cubeProperties.getID();
  }
