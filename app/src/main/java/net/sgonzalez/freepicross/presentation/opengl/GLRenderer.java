...
implements GLSurfaceView.Renderer {
  ...
  public void onDrawFrame(GL10 gl) {
    ...
    if (!GameManager.getInstance().isCleared() && this.mode != 0) {
      touchedCubeID = pickCube(gl);
    }
          ...
          if (cube[i][j][k].getID() == touchedCubeID) {
            switch (mode) {
              case 0:
                break;
              case 1:
                return cube[i][j][k].tryToMark();
              case 2:
                return cube[i][j][k].tryToDestroy();
            }
          }
        }
      }
  ...
  public boolean allCubesDestroyed() {
    for (int i = 0; i < nCubesX; i++) {
      for (int j = 0; j < nCubesY; j++) {
        for (int k = 0; k < nCubesZ; k++) {
          if (!cube[i][j][k].getSolid() && !cube[i][j][k].getDestroyed()) {
            return false;
          }
        }
      }
    ...
    for (int i = 0, id = 0; i < nCubesX; i++) {
      for (int j = 0; j < nCubesY; j++) {
        for (int k = 0; k < nCubesZ; k++, id++) {
          ...
          gl.glTranslatef((-1.0f * (nCubesX - 1 - hideX)) + (2 * i), (-1.0f * (nCubesY - 1)) + (2 * j),
          (1.0f * (nCubesZ - 1 + hideZ)) - (2 * k));
          //Draw the cube in red-scale
          if (isNotSurrounded(i, j, k)) {
            cube[i][j][k].drawColor(gl, id);
          }
          //Move back
          gl.glTranslatef((1.0f * (nCubesX - 1 - hideX)) - (2 * i), (1.0f * (nCubesY - 1)) - (2 * j),
          (-1.0f * (nCubesZ - 1 + hideZ)) + (2 * k));
  ...
  private void drawScene(GL10 gl) {
    ...
    if (!GameManager.getInstance().isCleared()) {
      switch (mode) {
        case 0:
          gl.glClearColor(0.25f, 0.75f, 0.25f, 1.0f);
          break;
        case 1:
          gl.glClearColor(0.25f, 0.50f, 0.75f, 1.0f);
          break;
        case 2:
          gl.glClearColor(0.75f, 0.50f, 0.55f, 1.0f);
          break;
      }
    } else {
      gl.glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    for (int i = 0; i < nCubesX; i++) {
      for (int j = 0; j < nCubesY; j++) {
        for (int k = 0; k < nCubesZ; k++) {
          ...
          //Draw the cube
          if (isNotSurrounded(i, j, k)) {
            cube[i][j][k].draw(gl);  //Draw the Cube
          }
          //Move back
          gl.glTranslatef((1.0f * (nCubesX - 1 - hideX)) - (2 * i), (1.0f * (nCubesY - 1)) - (2 * j),
          (-1.0f * (nCubesZ - 1 + hideZ)) + (2 * k));
  ...
  private boolean isNotSurrounded(int x, int y, int z) {
    if (x - 1 < 0 || x + 1 >= nCubesX) {
      return true;
    } else if (!cube[x - 1][y][z].getShown() || !cube[x + 1][y][z].getShown()) {
      return true;
    } else if (cube[x - 1][y][z].getDestroyed() || cube[x + 1][y][z].getDestroyed()) {
      return true;
    }
    if (y - 1 < 0 || y + 1 >= nCubesY) {
      return true;
    } else if (!cube[x][y - 1][z].getShown() || !cube[x][y + 1][z].getShown()) {
      return true;
    } else if (cube[x][y - 1][z].getDestroyed() || cube[x][y + 1][z].getDestroyed()) {
      return true;
    }
    if (z - 1 < 0 || z + 1 >= nCubesZ) {
      return true;
    } else if (!cube[x][y][z - 1].getShown() || !cube[x][y][z + 1].getShown()) {
      return true;
    } else if (cube[x][y][z - 1].getDestroyed() || cube[x][y][z + 1].getDestroyed()) {
      return true;
    }
    return false;
  }
  ...
  public void setHideZ(int hideZ) {
    this.hideZ = hideZ;
  }
