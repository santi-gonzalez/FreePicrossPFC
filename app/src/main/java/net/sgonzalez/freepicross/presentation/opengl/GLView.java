...
extends GLSurfaceView {
    ...
    if (event.getAction() == MotionEvent.ACTION_MOVE) {
      ...
      if (!GameManager.getInstance().isCleared()) {
        ...
        //Rotate around the axis otherwise
          ...
          if (!zooming && !hidding && mRenderer.getMode() == 0) {
            spin(dx, dy);
          }
        }
      }
  ...
  private void zoom(float dx) {
    ...
    if (mRenderer.getZ() < -75) {
      mRenderer.setZ(-75);
    }
    if (mRenderer.getZ() > -10) {
      mRenderer.setZ(-10);
    }
  ...
  private void hideZ() {
    ...
    if (distY > 100) {
      if (!hidden) {
        mRenderer.setHideZ(mRenderer.getHideZ() - 1);
      }
      hidden = true;
    } else if (distY < -100) {
      if (!hidden) {
    ...
    if (hidden) {
      ...
      if (mRenderer.getHideZ() < 0) {
        mRenderer.setHideZ(0);
      }
      if (mRenderer.getHideZ() > mRenderer.getNCubesZ() - 1) {
        mRenderer.setHideZ(mRenderer.getNCubesZ() - 1);
      }
    }
  }
  ...
  private void hideX() {
    ...
    if (distY > 100) {
      if (!hidden) {
        mRenderer.setHideX(mRenderer.getHideX() - 1);
      }
      hidden = true;
    } else if (distY < -100) {
      if (!hidden) {
    ...
    if (hidden) {
      mRenderer.setHideZ(0);
      if (mRenderer.getHideX() < 0) {
        mRenderer.setHideX(0);
      }
      if (mRenderer.getHideX() > mRenderer.getNCubesX() - 1) {
        mRenderer.setHideX(mRenderer.getNCubesX() - 1);
      }
    }
  }
  ...
  private void spin(float dx, float dy) {
    ...
    if (mRenderer.getXRot() > 90.0f) {
      mRenderer.setXRot(90.0f);
    }
    if (mRenderer.getXRot() < -90.0f) {
      mRenderer.setXRot(-90.0f);
    }
  ...
  private void hit() {
    switch (mRenderer.touchCube()) {
      case 0:
        break;
      case 1:
        SoundManager.getInstance().playSound(SoundManager.SFX_MARK);
        break;
      case 2:
        SoundManager.getInstance().playSound(SoundManager.SFX_MARK);
        break;
      case 3:
        SoundManager.getInstance().playSound(SoundManager.SFX_DESTROY);
        process();
        break;
      case 4:
        SoundManager.getInstance().playSound(SoundManager.SFX_ERROR);
  ...
  private boolean noMoreLifes() {
    if (GameManager.getInstance().getErrorsCur() >= GameManager.getInstance().getErrorsMax()) {
      return true;
    }
    return false;
  }
